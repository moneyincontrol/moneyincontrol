...
const FinanceApp = () => {
  ...
  useEffect(() => {
    ...
    console.log('FINNHUB_API_KEY:', FINNHUB_API_KEY.substring(0, 10) + '...' || 'NOT SET');
    console.log('NEWSAPI_KEY:', NEWSAPI_KEY.substring(0, 10) + '...' || 'NOT SET');
  }, []);
  ...
  const fetchGeneralNews = async () => {
    try {
      ...
      if (!NEWSAPI_KEY || NEWSAPI_KEY === 'YOUR_NEWSAPI_KEY') {
        console.log('NewsAPI key not set!');
        return [];
      }
      
      const url = `https://newsapi.org/v2/top-headlines?country=in&category=business&pageSize=20&apiKey=${NEWSAPI_KEY}`;
      console.log('General news URL:', url.substring(0, 100) + '...');
      ...
      console.log('General news articles:', data.articles?.length || 0);
  ...
  const fetchEarningsNews = async () => {
    try {
      console.log('Fetching earnings news...');
      if (!NEWSAPI_KEY || NEWSAPI_KEY === 'YOUR_NEWSAPI_KEY') return [];
      
      const url = `https://newsapi.org/v2/everything?q=India stock earnings results financial&sortBy=publishedAt&language=en&pageSize=20&apiKey=${NEWSAPI_KEY}`;
      console.log('Earnings news URL:', url.substring(0, 100) + '...');
      
      const res = await fetch(url);
      const data = await res.json();
      
      if (data.status !== 'ok') {
        console.error('Earnings API Error:', data.message);
        return [];
      }
      
      console.log('Earnings news articles:', data.articles?.length || 0);
      ...
      console.error('Error fetching earnings news:', err.message);
      return [];
    }
  };
  ...
  const fetchIPONews = async () => {
    try {
      console.log('Fetching IPO news...');
      if (!NEWSAPI_KEY || NEWSAPI_KEY === 'YOUR_NEWSAPI_KEY') return [];
      
      const url = `https://newsapi.org/v2/everything?q=India IPO listing initial public offering&sortBy=publishedAt&language=en&pageSize=20&apiKey=${NEWSAPI_KEY}`;
      console.log('IPO news URL:', url.substring(0, 100) + '...');
      
      const res = await fetch(url);
      const data = await res.json();
      
      if (data.status !== 'ok') {
        console.error('IPO API Error:', data.message);
        return [];
      }
      
      console.log('IPO news articles:', data.articles?.length || 0);
      ...
      console.error('Error fetching IPO news:', err.message);
      return [];
    }
  };
  ...
  const fetchMergerNews = async () => {
    try {
      console.log('Fetching M&A news...');
      if (!NEWSAPI_KEY || NEWSAPI_KEY === 'YOUR_NEWSAPI_KEY') return [];
      
      const url = `https://newsapi.org/v2/everything?q=India merger acquisition deal takeover&sortBy=publishedAt&language=en&pageSize=20&apiKey=${NEWSAPI_KEY}`;
      console.log('M&A news URL:', url.substring(0, 100) + '...');
      
      const res = await fetch(url);
      const data = await res.json();
      
      if (data.status !== 'ok') {
        console.error('M&A API Error:', data.message);
        return [];
      }
      
      console.log('M&A news articles:', data.articles?.length || 0);
      ...
      console.error('Error fetching M&A news:', err.message);
      return [];
    }
  };
  ...
  const fetchSectorNewsFunction = async (sectorName, keywords) => {
    try {
      console.log(`Fetching ${sectorName} news...`);
      if (!NEWSAPI_KEY || NEWSAPI_KEY === 'YOUR_NEWSAPI_KEY') return [];
      
      const url = `https://newsapi.org/v2/everything?q=${keywords}&sortBy=publishedAt&language=en&pageSize=20&apiKey=${NEWSAPI_KEY}`;
      console.log(`${sectorName} news URL:`, url.substring(0, 100) + '...');
      
      const res = await fetch(url);
      const data = await res.json();
      
      if (data.status !== 'ok') {
        console.error(`${sectorName} API Error:`, data.message);
        return [];
      }
      
      console.log(`${sectorName} news articles:`, data.articles?.length || 0);
      ...
      console.error(`Error fetching ${sectorName} news:`, err.message);
      return [];
    }
  };

  const fetchAllNews = async () => {
    console.log('Starting news fetch for all categories...');
    setNewsLoading(true);
    try {
  ...
  const sectors = [
    { name: 'IT', keyword: 'India IT stocks TCS Infosys Wipro technology' },
    { name: 'Banking', keyword: 'India bank stocks HDFC SBI ICICI banking financial' },
    { name: 'Pharma', keyword: 'India pharma stocks pharmaceutical drugs healthcare' },
  ...
  useEffect(() => {
    if (isAuthenticated && activeTab === 'news') {
      console.log('News tab activated, fetching all news...');
      fetchAllNews();
    }
          ...
          <div className="relative bg-white/5 backdrop-blur-3xl rounded-3xl border border-white/10 p-8 space-y-6">
            ...
            {authMode === 'phone' && authStep === 2 && confirmationResult && (
              <form onSubmit={handlePhoneOTPVerify} className="space-y-5">
                ...
                <button type="button" onClick={() => { setAuthStep(1); setVerificationCode(''); setConfirmationResult(null); }} className="w-full text-gray-400 hover:text-white text-sm">Back</button>
              </form>
            )}
        ...
        {activeTab === 'news' && (
          <div className="space-y-6">
            ...
            {newsLoading ? (
              ...
              <div className="bg-white/5 border border-dashed border-white/20 rounded-xl p-12 text-center">
                <p className="text-gray-300 text-lg">No news found. Click Refresh to load news.</p>
              </div>
            )}
          </div>
        )}
